        ...
        <div class="flex-none w-full max-w-full px-3">
            <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
                <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                    <h6 class="text-s font-bold">Edit Detail Barang {{ isset($data) ? $data->nama : '' }}</h6>
                </div>
                <div class="pb-6 pr-6 pl-6 flex-auto">
                    <form action="{{ route('admin.detail_barang.update', $data->id) }}" method="post" enctype="multipart/form-data" role="form">
                        ...
                        <label class="mb-2 ml-1 font-bold text-xs text-slate-700">Detail Barang</label>
                        <div class="mb-4">
                            <input type="number" class="flex-auto w-1/4 focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block appearance-none rounded-lg border border-solid border-gray-300 bg-csite bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow" id="inputDepo" placeholder="Masukkan Detail Barang" aria-label="depo" aria-describedby="depo-addon" autocomplete="off" value="{{ isset($data) ? $data->Detail : old('Detail') }}" name="Detail" required>
                            @error('Detail')
