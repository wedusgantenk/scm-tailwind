    ...
    <div class="flex-none w-full max-w-full px-3">
      <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
        <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
          <h6 class="text-s font-bold">Edit Detail Barang {{ $data->barang->nama }}</h6>
        </div>
          ...
          <form action="{{route('admin.detail_barang.update', $data->id)}}" method="post" enctype="multipart/form-data" role="form">
            ...
            <div class="mb-4">
                <input type="text"
                    ...
                    aria-describedby="detail_barang-addon" autocomplete="off" value="{{ isset($data) ? old('kode_unik', $data->kode_unik) : old('kode_unik') }}" name="kode_unik" required>
                @error('kode_unik')
                    <span id="detail_barang-addon" class="invalid-feedback" role="alert">{{ $message }}</span>
                @enderror
            </div>
