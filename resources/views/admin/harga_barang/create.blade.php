    ...
    <div class="flex-none w-full max-w-full px-3">
      <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
        <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
          <h6 class="text-s font-bold">Tambah Harga Barang</h6>
        </div>
        <div class="pb-6 pr-6 pl-6 flex-auto">
          <form action="{{ route('admin.harga_barang.store') }}" method="POST" enctype="multipart/form-data" role="form">
            @csrf
            ...
            <div class="mb-4">
                <input type="date" class="flex-auto w-1/4 focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow" id="inputTanggal" name="tanggal" required>
                @error('tanggal')
                    <span id="cluster-addon" class="invalid-feedback" role="alert">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
              <input type="number"
                ...
                id="inputDepo" placeholder="Masukkan Harga Barang" aria-label="depo" aria-describedby="depo-addon"
                autocomplete="off" value="{{ old('harga')}}" name="harga" required>
              @error('harga')
