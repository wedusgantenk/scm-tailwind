      ...
      <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
        ...
        <div class="pb-6 pr-6 pl-6 flex-auto">
          <form id="barang" action="{{ route('admin.barang.store') }}" method="POST" enctype="multipart/form-data" role="form">
            ...
            <div class="mb-4">
              <input type="text" class="flex-auto w-1/4 focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow" id="inputBarang" placeholder="Masukkan nama barang" aria-label="barang" aria-describedby="barang-addon" autocomplete="off" value="{{ old('nama')}}" name="nama" required>
              @error('nama')
            ...
            <div class="mb-4">
              <textarea class="flex-auto resize-none focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow" placeholder="Keterangan" aria-label="Alamat" aria-describedby="alamat-addon" name="keterangan" id="inputAlamat" rows="3">{{ old('alamat')}}</textarea>
            </div>
