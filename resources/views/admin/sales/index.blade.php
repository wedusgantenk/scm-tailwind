            ...
            <table class="px-6 py-3 table-auto items-center w-full mb-0 align-top border-transparent border-gray-200 text-slate-600">
              <thead class="align-bottom">
                <tr>
                  ...
                  <th class="px-6 py-3 font-bold text-left uppercase align-middle bg-transparent border-b border-gray-200 shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Depo</th>
                  ...
                  <th class="px-6 py-3 font-bold text-left uppercase align-middle bg-transparent border-b border-gray-200 shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Area</th>
              ...
              <tbody class="px-6 py-3 font-semibold text-left text-xs border-spacing-4">
                @forelse ($data as $d)
                  <tr>                                
                    <td class="text-center text-sm p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent"> {{ $loop->iteration }}. </td>
                    <td class="text-sm px-6 py-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent"> {{ $d->nama_depo }} </td>  
                    <td class="text-sm px-6 py-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent"> {{ $d->nama }} </td>
                    <td class="text-sm px-6 py-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent"> {{ $d->email }} </td>
                    <td class="text-sm px-6 py-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent"> {{ $d->area }} </td>
                    <td class="text-sm px-6 py-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent"> {{ $d->password }} </td>
                    <td class="text-sm px-6 py-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent"> {{ $d->status }} </td>                              
                    <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                      <a href="{{ route('admin.sales.edit', $d->id) }}"
                        class="mx-2 btn btn-success btn-xs"><i class="fas fa-edit fa-lg"></i></a>
                      <a href="{{ route('admin.sales.delete', $d->id) }}"
                        class="mx-1 btn btn-danger btn-xs"
                        onclick="return confirm('Apakah yakin ingin menghapus sales {{ $d->nama }} ?');"><i
                        class="fas fa-trash-alt fa-lg"></i></a>                                    
                      </td>
                  </tr>         
                    @empty
                    <tr>
                      <th colspan="8" class="p-2 text-center bg-transparent border-b whitespace-nowrap shadow-transparent">Tidak ada data</th>
                    </tr>
                @endforelse     
              </tbody>
            </table>
